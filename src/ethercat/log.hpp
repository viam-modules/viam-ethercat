...
// Those sites are marked "RT-context log": the sink still allocates, takes the SDK logger's mutexes
// and writes to stdout on the calling thread; if that ever shows up as jitter, route them through a
// lock-free ring drained by a non-RT thread.
